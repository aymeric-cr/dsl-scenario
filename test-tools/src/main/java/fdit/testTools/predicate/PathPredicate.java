package fdit.testTools.predicate;
...
import static fdit.testTools.predicate.StringPredicate.equalsIgnoringWhitespaces;
...
public final class PathPredicate {
    ...
    public static Predicate<Path> filepath(final Predicate<String> predicate) {
        return new Predicate<Path>() {
            public boolean test(final Path element) {
                return predicate.test(separatorsToUnix(element.toAbsolutePath().toString()));
            }
        ...
        return path -> {
            if (isRegularFile(path)) {
                if (path.getFileName().toString().equals(name)) {
                    ...
                    System.err.println(readFileToString(path));
                }
            }
            return false;
        };
    ...
    public static Predicate<Path> containsAll(final String... contents) {
        return file -> {
            final String fileToString = readFileToString(file);
    ...
    public static Collection<Path> listPaths(final Path directory) {
        try {
            return list(directory).collect(toList());
        } catch (final IOException e) {
